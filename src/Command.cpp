...
{
    return std::string_view{&line[KEYWORD_BEGIN], std::min(line.find_first_of(' '), KEYWORD_LENGTH_MAX)};
}


std::string_view extractAddress(const std::string &line)
{
    return std::string_view{&line[ADDRESS_BEGIN],
...
{
    return std::string_view{&line[DATA_WORD_BEGIN],
...
{
    return std::string_view{&line[ORDER_BEGIN], ORDER_LENGTH_MAX};
}


std::string_view extractStatus(const std::string &line)
{
    return std::string_view{&line[STATUS_BEGIN],
